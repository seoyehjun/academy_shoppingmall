...
//	alert("모두 주문하기");
			var order_goods_qty;
			var order_goods_id;
			var objForm=document.frm_order_all_cart;//객체스크립트단에서 <form>태그 받아서 post해줄셈인모양이다.
			var cart_goods_qty=objForm.cart_goods_qty;//장바구니 개개의 갯수 리스트이다.
			var h_order_each_goods_qty=objForm.h_order_each_goods_qty;
			var checked_goods=objForm.checked_goods;//<input>체크박스
			...
			if(length>1){
				for(var i=0; i<length;i++){
					if(checked_goods[i].checked==true){
						order_goods_id=checked_goods[i].value;//체크박스이지만 value는 상품id이다.
						order_goods_qty=cart_goods_qty[i].value;//장바구니 개개의 갯수 <input>형식이다.
						cart_goods_qty[i].value="";//원래 개별 주문갯수였는데 원하는 형식으로 바꿀예정
						cart_goods_qty[i].value=order_goods_id+":"+order_goods_qty;
						//<form>태그의 cart_good_qty를 '인덱스 : 개당주문수량' 형식으로 바꾸어준다.
